		...
		const request = transport.request(
			...
			response => {
				...
				response.once('aborted', () =>
					reject(localProxyError(requestId, 'unknown', new Error('Proxy response was aborted.'))),
				);
				response.once('error', error => reject(localProxyError(requestId, 'unknown', error)));
			},
		);
		request.once('socket', socket => {
			if (!socket.connecting) dispatched = true;
			else if (endpoint.protocol === 'https:') socket.once('secureConnect', () => (dispatched = true));
			else socket.once('connect', () => (dispatched = true));
		});
		request.once('error', error =>
			reject(localProxyError(requestId, dispatched ? 'unknown' : 'not_dispatched', error)),
		);
...
export class ProxyApiHandler extends ApiHandler {
	...
	override async request<T = unknown>(
		...
		const result = await requestProxy(this.endpoint, this.credential, encoded, requestId);
