...
describe('world threads', () => {
	test('registers a thread under a channel, distinct from a normal channel', async () => {
		...
		const view = bot.cachedGuild(guild.id);
	...
	test('a runtime-created thread coexists with a seeded one and carries metadata', async () => {
		...
		const threads = bot.cachedGuild(guild.id)?.threads ?? [];
		expect(threads.map(t => t.name).sort()).toEqual(['runtime-thread', 'seeded-thread']);
		const runtime = bot.cachedGuild(guild.id)?.thread('runtime-thread');
...
describe('world voice states', () => {
	test('registers a voice state readable via bot.voiceState and the cache', async () => {
		const world = mockWorld();
		...
		const voice = bot.voiceState(guild.id, member.user.id);
		...
		expect(bot.voiceState(guild.id, 'absent-user')).toBeUndefined();
	});
