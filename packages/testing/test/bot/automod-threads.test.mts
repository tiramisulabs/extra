...
describe('automod rules', () => {
	...
	test('create, edit and delete drive the world state', async () => {
		...
		expect(bot.state.autoModRule(guild.id, id)).toMatchObject({ name: 'no-links', enabled: false });
		...
		expect(bot2.state.autoModRule(guild.id, id)).toBeUndefined();
		await bot.close();
		await bot2.close();
	});
});
...
describe('thread membership', () => {
	test('join resolves @me to the bot and listMembers returns the joined set', async () => {
		...
		expect(bot.state.threadMembers(thread.id).sort()).toEqual([TEST_BOT_ID, 'user-7'].sort());
		await bot.close();
	});
	...
	test('leave removes the bot from the thread', async () => {
		...
		expect(bot.state.threadMembers(thread.id)).not.toContain(TEST_BOT_ID);
		await bot.close();
	});
});
...
describe('active threads', () => {
	test('listGuildActive returns only non-archived threads', async () => {
		...
		expect(bot.state.activeThreads(guild.id)).toHaveLength(1);
		await bot.close();
	});
});
